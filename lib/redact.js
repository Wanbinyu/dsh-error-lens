...
export function classifyFailure(failure) {
    ...
    if (status === 401 || code === 'AUTH' || code === 'UNAUTHORIZED')
        return 'authentication';
    if (status === 403 || code === 'FORBIDDEN')
        return 'forbidden';
    if (status === 429 || code.includes('RATE_LIMIT'))
        return 'rate-limit';
    if (code.includes('CONTEXT') || /context (?:length|window)|maximum context|too many tokens/.test(message))
        return 'context-limit';
    if (/developer role|unsupported role|messages?\.role|unknown tool\s*["']{0,2}|tool[_ -]?call|function[_ -]?call/.test(message)
        || code === 'UNKNOWN_TOOL')
        return 'compatibility';
    if (code.includes('TIMEOUT') || /timed? out|timeout/.test(message))
        return 'timeout';
    if (code === 'TRANSPORT' || /econnreset|econnrefused|fetch failed|network error|socket hang up/.test(message))
        return 'transport';
    if ((status !== undefined && status >= 500) || code === 'SERVER')
        return 'server';
